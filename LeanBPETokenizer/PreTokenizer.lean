...
def nextTokenLen (data : ByteArray) (pos : Nat) : Nat :=
...
theorem nextTokenLen_pos (data : ByteArray) (pos : Nat) :
    1 ≤ nextTokenLen data pos := by
  simp only [nextTokenLen, bne_iff_ne, ne_eq]
  split_ifs <;> omega
...
theorem preTokenize_partition (data : ByteArray) :
    (preTokenizeASCII data).foldl (· ++ ·) ByteArray.empty = data := by
  ...
  suffices key : ∀ (n pos : Nat) (acc : Array ByteArray),
      ...
      (preTokenizeASCII.loop data pos acc).foldl (· ++ ·) ByteArray.empty = data by
  ...
  | zero =>
      ...
      have hloop : preTokenizeASCII.loop data pos acc = acc := by
  ...
  | succ n ih =>
      ...
      · -- Still within data: take one token and recurse
        set n_tok := nextTokenLen data pos with hn_tok
        set endPos := min (pos + n_tok) data.size with hendPos
        have h_advance : pos < endPos := by
          have := nextTokenLen_pos data pos
          simp only [endPos, Nat.min_def]
          split_ifs with hle'
          · omega
          · exact h_lt
        have hloop : preTokenizeASCII.loop data pos acc =
            preTokenizeASCII.loop data endPos (acc.push (data.extract pos endPos)) := by
